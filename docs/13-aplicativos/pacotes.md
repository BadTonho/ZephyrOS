...
`api` usa `0.6` em pacotes novos; `0.3`, `0.4` e `0.5` continuam aceitos por
compatibilidade append-only. `entry` e sempre `APP.ZAP`. `dependencies` e vazio ou
...
O `app.json` precisa de `id`, `name` e `version`; `api` assume `0.6`, aceita
explicitamente `0.3`, `0.4` e `0.5` para pacotes legados e
...
0.4, 0.5 e 0.6, corrupcao de CRC32 e injecao em uma imagem temporaria. `package-demo`
