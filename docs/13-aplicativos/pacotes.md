...
`api` usa `0.5` em pacotes novos; `0.3` e `0.4` continuam aceitos por
...
O `app.json` precisa de `id`, `name` e `version`; `api` assume `0.5`, aceita
explicitamente `0.3` e `0.4` para pacotes legados e
...
0.4 e 0.5, corrupcao de CRC32 e injecao em uma imagem temporaria. `package-demo`
