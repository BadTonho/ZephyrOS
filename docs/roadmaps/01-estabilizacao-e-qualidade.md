...
- [x] Preservar `appcheck`, `q2check`, `app outputtest [fail]`,
  `app inputtest` por `Enter` e a matriz de interfaces como verificacoes
  detalhadas separadas.

Validado pelo usuario: `make q3check` e `make clean && make` passaram sem
warnings novos. No QEMU, `regcheck` concluiu em `OK` apos `F12`; uso invalido,
`procs`, `health`, `schedcheck` e `memcheck` confirmaram prompt unico e
ausencia de processos, zumbis ou diretorios de usuario residuais.
