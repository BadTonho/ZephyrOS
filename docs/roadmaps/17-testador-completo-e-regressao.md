...
- [x] A fixture `host:kernel:runtime` exercita diretamente as rotas finitas
      ...
      mouse, solicitações ao Shell, redesenho e DHCP. As entradas de processo
      executam uma única iteração host-only limitada; o build normal mantém
      os loops cooperativos permanentes.
...
- [x] A fixture `host:shell:checks` passou a exercitar os caminhos de
      `appcheck`, `q2check`, `regcheck` e `blkcheck`, incluindo dispatchers,
      jobs, loader, migração de aplicativos, fixtures de entrada, validações
      de serviços, scheduler, dispositivos, USB, ACPI, energia e pacotes.
- [x] Os doubles permanecem estáticos e controlados: erros de dependência,
      recursos indisponíveis, snapshots inconsistentes e cancelamentos são
      retornados sem iniciar processos, acessar disco, rede ou hardware reais.
- [x] `make test-shell-checks-host
      HOST_CC=C:\\msys64\\ucrt64\\bin\\gcc.exe` passou; a cobertura dinâmica
...
### Incremento Shell Checks: estados, cancelamento e snapshots de energia — 2026-09-05

- [x] A fixture `host:shell:checks` passou a exercitar estados de job,
      drenagem, finalização, bloqueio de entrada e cancelamento cooperativo,
      incluindo a rejeição de scancode fora da fase de cancelamento.
- [x] Foram validados snapshots de energia ACPI, limites do registrador de
      reset e coerência de recovery para estados `DISABLED`, `DEGRADED` e
      `READY`, usando apenas doubles estáticos.
- [x] `make test-shell-checks-host
      HOST_CC=C:\\msys64\\ucrt64\\bin\\gcc.exe` passou; a cobertura dinâmica
      observou 64 funções reais de `src/shell/shell_checks.c`, sem endereços
      desconhecidos ou símbolos ambíguos. A etapa também passou por
      `make q3check`, build limpo, sincronização/renderização do catálogo,
...
- [x] Foi criada a fixture `host:ui:updater`, com contrato interno ativado
      somente por `ZEPHYROS_HOST_TEST`; o caminho normal do System Updater não
...
### Incremento Shell Checks: verificação e limpeza de imagens — 2026-09-05

- [x] A fixture `host:shell:checks` passou a verificar imagens ZAPP após
      gravação, cobrindo leitura válida, tamanho divergente, erro de disco,
      corrupção de conteúdo e argumentos inválidos.
- [x] A mesma fixture exercitou a limpeza limitada de imagens, incluindo
      remoção parcial, saturação do limite e caminho nulo, sem tocar no
      filesystem real.
- [x] `make test-shell-checks-host
      HOST_CC=C:\\msys64\\ucrt64\\bin\\gcc.exe` passou; o relatório
      instrumentado terminou `PASS`, observando 47 funções reais de
      `src/shell/shell_checks.c`, sem endereços desconhecidos ou símbolos
      ambíguos. A sincronização do catálogo passou e marcou 2 superfícies
      adicionais como `COVERED`, totalizando 7.326 superfícies, 6.881
      `COVERED`, 445 `PENDING`, 59 aposentadas e 165 casos.

### Incremento Shell Checks: Q2Check de faults e limpeza — 2026-09-05

- [x] A fixture `host:shell:checks` passou a validar o contrato do Q2Check
      para contagem e resumo de page fault, incluindo fault ausente, PID ou
      contagem incorreta, resumo inválido e erro de consulta.
- [x] O mesmo caso validou a limpeza de foco, usuários e zombies, além da
      rejeição de índice inválido, falha de criação e início válido do fixture
      de fault, com snapshots estáticos e sem criar processos reais.
- [x] `make test-shell-checks-host
      HOST_CC=C:\\msys64\\ucrt64\\bin\\gcc.exe` passou; o relatório
      instrumentado terminou `PASS`, observando 50 funções reais de
      `src/shell/shell_checks.c`, sem endereços desconhecidos ou símbolos
...
- [x] A fixture host-only `host:shell:network-checks` passou a exercitar os
      handlers, dispatchers, jobs cooperativos, esperas limitadas, seleção de
      sockets e formatadores de rede de `shell_commands_network.c`, incluindo
      entradas válidas, estados vazios, argumentos inválidos e dependências
      indisponíveis.
- [x] Foram usados doubles estáticos para rede, processos, VFS, IPC, vídeo,
      timer e jobs. Nenhum hardware, rede externa, armazenamento ou allocator
      real foi acessado; a execução não aceita retry nem espera indefinida.
- [x] `make test-shell-network-checks-host HOST_CC=C:\\msys64\\ucrt64\\bin\\gcc.exe`
      passou com `-Wall -Wextra -Werror` e cobertura dinâmica `PASS`, com 148
      superfícies C de rede observadas no relatório, incluindo o helper
      host-only, sem endereços desconhecidos ou símbolos ambíguos. O catálogo
      registra 7.314 superfícies, 6.706 `COVERED`, 608 `PENDING`, 59 aposentadas
      e 161 casos; as pendências de outros subsistemas continuam explícitas.
...
### Evidência Shell/input e hosted regenerada — 2026-09-05

- [x] Os casos existentes `host:shell:input` e `host:shell:hosted` foram
      reexecutados após o build limpo, com fixtures estáticas e sem hardware.
- [x] Os relatórios dinâmicos terminaram `PASS`, sem endereços desconhecidos ou
      símbolos ambíguos, observando as duas superfícies internas de
      `src/shell/shell_input.c` e as três de `src/shell/shell_hosted.c`.
- [x] A sincronização, renderização, `make catalog-test` e os dois alvos
