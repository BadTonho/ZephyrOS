...
- Incremento Shell/diagnostics commands — concluído em 2026-09-04
  ...
  que chama diretamente os dispatchers reais de `pwd`, `cd`, `mouse` e `log`.
  Foram validados caminhos válidos, argumentos extras, limites de token e
  velocidade, estados indisponíveis, níveis, histórico e preservação da
  configuração após rejeição. O alvo passou com `HOST_CC`, `-Wall -Wextra -Werror`,
