...
despacho assíncrono, mouse, solicitações ao Shell, redesenho, DHCP e uma
iteração limitada de cada entrada de processo. Ele usa dependências estáticas
falsas; `kernel_main` não é iniciado. O relatório fica em
`build/test-results/kernel-host/`; somente a inicialização completa de
`kernel_main` permanece pendente para um executor QEMU real.
...
O agregado `test-shell3-host` cobre os contratos diretamente afetados de VFS,
Storage, permissoes, pipeline, indice, Explorer, Task Manager, Settings,
Desktop, WM, Taskbar, entrada, aplicativos e black-box:

```text
make test-shell3-host HOST_CC=C:\\msys64\\ucrt64\\bin\\gcc.exe
make test-shell3-qemu SHELL3_QEMU_WORKERS=4 SHELL3_QEMU_SEED=2203
```

Os resultados ficam em `build/test-results/shell3/`. O caso dedicado usa a tag
`shell3`, snapshot descartavel e valida `pwd`, `cd`, `ls`, `cat`, pipelines,
...
tentativas de escrita somente leitura. Ela tambem executa `devices -v` e
...
O alvo `test-appstore-host` valida os contratos determinísticos da App Store
com backends estáticos, sem acessar armazenamento, rede ou hardware reais:
...
desenho Simple/Classic, navegação remota, confirmação, teclado, mouse,
callbacks hospedados, inicialização, refreshes, preflight, operações locais e
workers remotos. O relatório instrumentado fica em
`build/test-results/appstore-host/coverage.json` e deve terminar com
`status=PASS`, `unknown_addresses=[]` e `ambiguous_symbols=[]`. O worker é
executado em passo único no host para impedir loop infinito; a inicialização,
...
`status=PASS`, `unknown_addresses=[]` e `ambiguous_symbols=[]`, observando 87
funções reais de `src/updater/updater.c` e quatro funções compartilhadas de
`src/core/string.c`. A fixture também cobre o ciclo de vida Simple/Classic,
desenho, callbacks hospedados, teclado, mouse, refreshes, preflight,
operações locais e worker remoto em passo único. Os cinco helpers declarados
em `ui/updater_test.h` permanecem vinculados ao caso como contrato de teste.
