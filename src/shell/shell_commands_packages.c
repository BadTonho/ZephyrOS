...
void shell_dispatch_cmd_pkgcheck(const char* arguments);
...
int shell_packages_host_test_contracts(void) {
    ...
    char truncated[4];
    char package_id[16];
    char mutable_value[FS_MAX_PATH];
    update_version_t version;
    update_action_result_t update_action;
    update_history_entry_t update_history;
    update_remote_candidate_t update_candidate;
    update_remote_result_t update_remote_result;
    update_remote_runtime_result_t runtime_remote_result;
    app_package_info_t package_info;
    app_catalog_entry_t catalog_entry;
    app_package_plan_t package_plan;
    app_package_action_result_t package_action;
    app_remote_entry_t remote_entry;
    app_remote_options_t remote_options;
    shell_job_context_t job_context;
    int confirmed_fixture = 0;
    int downgrade_fixture = 0;
    uint8_t hash[CRYPTO_SHA256_SIZE];
    uint32_t capabilities = APP_CATALOG_CAPABILITY_VERIFY |
                            APP_CATALOG_CAPABILITY_INSTALL |
                            APP_CATALOG_CAPABILITY_RUN |
    ...
    version.minor = 2U;
    version.patch = 3U;
    kmemset(&update_action, 0, sizeof(update_action));
    update_action.entry_count = 1U;
    update_action.completed_entries = 1U;
    update_action.reason = UPDATE_ACTION_NONE;
    update_action.from_version = version;
    update_action.to_version = version;
    update_action.from_epoch = 10U;
    update_action.to_epoch = 11U;
    kmemset(&update_history, 0, sizeof(update_history));
    update_history.sequence = 1U;
    update_history.operation = UPDATE_HISTORY_OPERATION_APPLY;
    update_history.outcome = UPDATE_HISTORY_OUTCOME_SUCCESS;
    update_history.action_reason = UPDATE_ACTION_NONE;
    update_history.verification_reason = ZUPD_REASON_NONE;
    update_history.from_version = version;
    update_history.to_version = version;
    update_history.entry_count = 1U;
    update_history.completed_entries = 1U;
    update_history.reboot_required = 1U;
    kmemcpy(update_history.package_alias, "UPDATE.ZUP", 11U);
    kmemset(&update_candidate, 0, sizeof(update_candidate));
    update_candidate.generation = 2U;
    update_candidate.base_version = version;
    update_candidate.target_version = version;
    update_candidate.base_epoch = 10U;
    update_candidate.target_epoch = 11U;
    update_candidate.package_size = 128U;
    kmemcpy(update_candidate.package_path, "UPDATE.ZUP", 11U);
    kmemset(hash, 0x5AU, sizeof(hash));
    kmemcpy(update_candidate.package_hash, hash, sizeof(hash));
    kmemset(&update_remote_result, 0, sizeof(update_remote_result));
    update_remote_result.release.tag[0] = 'v';
    kmemcpy(update_remote_result.release.tag, "v1.2.3", 7U);
    kmemcpy(update_remote_result.release.release_id, "release-1", 10U);
    kmemcpy(update_remote_result.release.release_name, "Release 1", 10U);
    kmemcpy(update_remote_result.release.source_commit, "commit-1", 9U);
    update_remote_result.release.package_size = 128U;
    update_remote_result.release.api_metadata_present = 1U;
    update_remote_result.candidate = update_candidate;
    update_remote_result.secure = 1U;
    update_remote_result.tls_verified = 1U;
    update_remote_result.http_status = 200U;
    update_remote_result.bytes_received = 128U;
    update_remote_result.redirect_count = 1U;
    kmemcpy(update_remote_result.cached_alias, "UPDATE.ZPK", 11U);
    kmemset(&runtime_remote_result, 0, sizeof(runtime_remote_result));
    runtime_remote_result.manifest.target_version = version;
    runtime_remote_result.manifest.target_epoch = 11U;
    runtime_remote_result.manifest.entry_count = 1U;
    runtime_remote_result.http_status = 200U;
    runtime_remote_result.bytes_received = 128U;
    runtime_remote_result.assets_downloaded = 1U;
    runtime_remote_result.assets_reused = 1U;
    kmemcpy(runtime_remote_result.cached_manifest_alias, "ZUM2.MAN", 9U);
    kmemcpy(runtime_remote_result.cached_package_alias, "ZUM2.ZPK", 9U);
    kmemset(&package_info, 0, sizeof(package_info));
    kmemcpy(package_info.id, "TESTAPP", 8U);
    kmemcpy(package_info.name, "Test App", 9U);
    kmemcpy(package_info.version, "1.0", 4U);
    package_info.dependency_count = 1U;
    kmemcpy(package_info.dependencies[0], "BASEAPP", 8U);
    kmemset(&catalog_entry, 0, sizeof(catalog_entry));
    kmemcpy(catalog_entry.alias, "TESTAPP.ZPK", 12U);
    catalog_entry.source = package_info;
    catalog_entry.installed = package_info;
    catalog_entry.source_size = 128U;
    catalog_entry.capabilities = capabilities;
    catalog_entry.missing_dependency_mask = 1U;
    catalog_entry.state = APP_CATALOG_STATE_UPDATE_AVAILABLE;
    catalog_entry.reason = APP_CATALOG_REASON_NONE;
    catalog_entry.has_source = 1U;
    catalog_entry.has_installed = 1U;
    kmemset(&package_plan, 0, sizeof(package_plan));
    package_plan.entry_count = 1U;
    package_plan.target_index = 0U;
    package_plan.entries[0].action = APP_PACKAGE_PLAN_ACTION_UPDATE;
    kmemcpy(package_plan.entries[0].id, "TESTAPP", 8U);
    kmemcpy(package_plan.entries[0].alias, "TESTAPP.ZPK", 12U);
    kmemcpy(package_plan.entries[0].from_version, "0.9", 4U);
    kmemcpy(package_plan.entries[0].to_version, "1.0", 4U);
    kmemset(&package_action, 0, sizeof(package_action));
    package_action.reason = APP_PACKAGE_ACTION_REASON_NONE;
    package_action.info = package_info;
    package_action.plan = package_plan;
    package_action.blocker_count = 1U;
    kmemcpy(package_action.blocker_ids[0], "BASEAPP", 8U);
    package_action.required_clusters = 2U;
    package_action.free_clusters = 4U;
    kmemset(&remote_entry, 0, sizeof(remote_entry));
    remote_entry.info = package_info;
    remote_entry.package_size = 128U;
    remote_entry.state = APP_REMOTE_ENTRY_AVAILABLE;
    remote_entry.reason = APP_REMOTE_REASON_NONE;
    remote_entry.cached = 1U;
    remote_entry.installed = 1U;
    kmemcpy(remote_entry.package_path, "TESTAPP.ZPK", 12U);
    kmemcpy(remote_entry.installed_version, "1.0", 4U);
    kmemset(&remote_options, 0, sizeof(remote_options));
    kmemset(&job_context, 0, sizeof(job_context));
    kmemcpy(job_context.arguments, "verify UPDATE.ZUP", 19U);

    cmd_update_print_version("Version: ", &version, 11U);
    cmd_update_verify("UPDATE.ZUP");
    (void)cmd_update_cancel_check(0);
    cmd_update_refresh_component();
    cmd_update_print_action(&update_action);
    cmd_update_print_version_inline(&version, 11U);
    cmd_update_print_history_entry(&update_history);
    cmd_update_print_capability("cap=", 1);
    cmd_update_status();
    cmd_update_history();
    cmd_update_apply("UPDATE.ZUP", 0);
    cmd_update_apply("UPDATE.ZUP", 1);
    cmd_update_rollback(0);
    cmd_update_rollback(1);
    cmd_update_test_fail_after("1");
    cmd_update_test_fail_after("0");
    cmd_update_print_remote_candidate(&update_candidate);
    cmd_update_print_hash(hash);
    cmd_update_print_release_result(&update_remote_result);
    cmd_update_remote_status();
    cmd_update_remote_control("status", 0);
    cmd_update_remote_control("enable", 0);
    cmd_update_remote_control("disable", 0);
    cmd_update_remote_control("clear", 0);
    cmd_update_remote_control("clear", 1);
    cmd_update_fetch(0, 0);
    cmd_update_fetch("https://example.invalid/update", 1);
    cmd_update_github("check", "v1.2.3", 0);
    cmd_update_github("fetch", "v1.2.3", 1);
    cmd_update_print_runtime_result(&runtime_remote_result, OK);
    cmd_update_runtime_status();
    cmd_update_runtime_check(0);
    cmd_update_runtime_check("v1.2.3");
    cmd_update_runtime_fetch(0, 0, 0);
    cmd_update_runtime_fetch("v1.2.3", 1, 1);
    cmd_update_runtime_verify("UPDATE.ZUP");
    cmd_update_runtime_action(0, 0);
    cmd_update_runtime_action(1, 1);
    cmd_update_runtime_clear(0);
    cmd_update_runtime_clear(1);
    cmd_update_runtime_test_fail_after("1");
    cmd_update_system_print_result(OK);
    cmd_update_system_print_result(ERR_INVALID);
    cmd_update_system_verify("SYSTEM.ZSY");
    cmd_update_system_status();
    cmd_update_system_fetch("v1.2.3", 0);
    cmd_update_system_fetch("v1.2.3", 1);
    cmd_update_system_apply(0);
    cmd_update_system_apply(1);
    cmd_update_system_cancel(0);
    cmd_update_system_cancel(1);
    cmd_update_system_check("v1.2.3");
    cmd_update_system_slots_print_slot(0U, &shell_update_workspace.system_slots_status.slots[0]);
    cmd_update_system_slots();
    cmd_update_system_stage("SYSTEM.ZSY", 0);
    cmd_update_system_stage("SYSTEM.ZSY", 1);
    cmd_update_runtime("runtime status");
    cmd_update_runtime("runtime check --tag v1.2.3");
    cmd_update_runtime("runtime fetch --tag v1.2.3 --full --confirm");
    cmd_update_runtime("runtime verify UPDATE.ZUP");
    cmd_update_runtime("runtime apply --confirm");
    cmd_update_runtime("runtime rollback");
    cmd_update_runtime("runtime clear --confirm");
    cmd_update_runtime("runtime test fail-after 1");
    cmd_update_system("system status");
    cmd_update_system("system verify SYSTEM.ZSY");
    cmd_update_system("system fetch --tag v1.2.3 --confirm");
    cmd_update_system("system apply --confirm");
    cmd_update_system("system cancel");
    cmd_update_system("system check --tag v1.2.3");
    cmd_update_system("system slots");
    cmd_update_system("system stage SYSTEM.ZSY --confirm");
    cmd_update("status");
    cmd_update("history");
    cmd_update("verify UPDATE.ZUP");
    cmd_update("apply UPDATE.ZUP --confirm");
    cmd_update("rollback");
    cmd_update("test fail-after 1");
    cmd_update("remote status");
    cmd_update("github check --tag v1.2.3");
    cmd_update("fetch --url https://example.invalid/update --confirm");
    cmd_pkg_print_info(&package_info);
    cmd_pkg_print_info(0);
    cmd_pkg_print_usage();
    cmd_pkg_list();
    kmemcpy(mutable_value, "testapp", 8U);
    cmd_pkg_info(mutable_value);
    cmd_pkg_verify("PACKAGE.ZPK");
    cmd_pkg_install("PACKAGE.ZPK");
    kmemcpy(mutable_value, "testapp", 8U);
    cmd_pkg_remove(mutable_value);
    cmd_pkg(0);
    cmd_pkg("list");
    cmd_pkg("info TESTAPP");
    cmd_pkg("verify PACKAGE.ZPK");
    cmd_pkg("install PACKAGE.ZPK");
    cmd_pkg("remove TESTAPP");
    cmd_pkgcheck();
    cmd_store_print_capabilities(capabilities);
    cmd_store_print_capabilities(APP_CATALOG_CAPABILITY_NONE);
    cmd_store_print_dependencies(&catalog_entry);
    catalog_entry.source.dependency_count = 0U;
    cmd_store_print_dependencies(&catalog_entry);
    catalog_entry.source.id[0] = '\0';
    cmd_store_print_dependencies(&catalog_entry);
    catalog_entry.source = package_info;
    cmd_store_print_list_entry(&catalog_entry);
    cmd_store_print_usage();
    cmd_store_status();
    cmd_store_list();
    kmemcpy(mutable_value, "TESTAPP", 8U);
    cmd_store_info(mutable_value);
    cmd_store_print_plan(&package_plan);
    cmd_store_print_action_blockers(&package_action);
    cmd_store_print_action_result("Action: ", OK);
    cmd_store_print_action_result("Action: ", ERR_INVALID);
    kmemcpy(mutable_value, "TESTAPP", 8U);
    (void)cmd_store_find_action_entry(mutable_value);
    cmd_store_refresh_after_mutation();
    cmd_store_set_local_failure(APP_PACKAGE_ACTION_REASON_READ_ERROR);
    cmd_store_print_failed_transaction(0);
    cmd_store_print_failed_transaction(1);
    kmemcpy(mutable_value, "TESTAPP.ZPK", 12U);
    (void)cmd_store_build_plan(mutable_value, 0, 0);
    (void)cmd_store_build_plan(mutable_value, 1, 1);
    cmd_store_install(mutable_value, 0);
    cmd_store_install(mutable_value, 1);
    cmd_store_update(mutable_value, 0, 0);
    cmd_store_update(mutable_value, 1, 1);
    cmd_store_remove(mutable_value, 0);
    cmd_store_remove(mutable_value, 1);
    cmd_store_rollback(mutable_value, 0);
    cmd_store_rollback(mutable_value, 1);
    cmd_store_history(0);
    cmd_store_history("TESTAPP");
    cmd_store_test_fail_after("1");
    cmd_store_run("TESTAPP", "--arg");
    cmd_store_remote_status();
    cmd_store_remote_print_entry(&remote_entry);
    cmd_store_remote_list();
    kmemcpy(mutable_value, "TESTAPP", 8U);
    cmd_store_remote_info(mutable_value);
    cmd_store_remote_print_result("Remote: ", OK);
    cursor = " --confirm --downgrade --url https://example.invalid";
    (void)cmd_store_remote_options(&cursor, 1, &confirmed_fixture,
                                   &downgrade_fixture);
    (void)cmd_store_remote_control("status", "", &remote_options);
    (void)cmd_store_remote_control("enable", "", &remote_options);
    (void)cmd_store_remote_control("check", "--url https://example.invalid",
                                   &remote_options);
    (void)cmd_store_remote_control("clear", "--confirm", &remote_options);
    cmd_store_remote("status");
    cmd_store_remote("list");
    cmd_store_remote("info TESTAPP");
    cmd_store_remote("check --url https://example.invalid");
    cmd_store_remote("test fail-after 1");
    cmd_store_remote("fetch TESTAPP --confirm");
    cmd_store("status");
    cmd_store("list");
    cmd_store("info TESTAPP");
    cmd_store("install TESTAPP --confirm");
    cmd_store("update TESTAPP --downgrade --confirm");
    cmd_store("remove TESTAPP --confirm");
    cmd_store("rollback TESTAPP --confirm");
    cmd_store("history TESTAPP");
    cmd_store("test fail-after 1");
    cmd_store("run TESTAPP --arg");
    cmd_store("remote status");
    cmd_store(0);
    (void)shell_packages_job_step(0);
    job_context.generation = shell_packages_job_generation;
    shell_packages_job_operation = SHELL_PACKAGES_JOB_PKG;
    (void)shell_packages_job_step(&job_context);
    shell_packages_job_operation = SHELL_PACKAGES_JOB_STORE;
    (void)shell_packages_job_step(&job_context);
    shell_packages_job_operation = SHELL_PACKAGES_JOB_UPDATE;
    (void)shell_packages_job_step(&job_context);
    ...
    shell_packages_job_operation = SHELL_PACKAGES_JOB_PKG;
    (void)shell_packages_job_step(&job_context);
    shell_packages_job_finish(&job_context, SHELL_JOB_STATE_CANCELLED,
                              ERR_TIMEOUT);
    shell_packages_job_finish(&job_context, SHELL_JOB_STATE_FAILED,
                              ERR_STATE);
    (void)shell_packages_job_cancel(&job_context);
    (void)shell_packages_job_drain(&job_context);
    (void)shell_packages_start_job("pkg", "verify PACKAGE.ZPK");
    (void)shell_packages_start_job("store", "install TESTAPP");
    (void)shell_packages_start_job("update", "apply UPDATE.ZUP");
    shell_dispatch_cmd_pkg("list");
    shell_dispatch_cmd_store("status");
    shell_dispatch_cmd_update("status");
    shell_dispatch_cmd_pkgcheck(0);
