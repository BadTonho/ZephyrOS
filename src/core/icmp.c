...
#define ICMP_MAX_TICK_INTERVAL 0xFFFFFFFFU
...
static uint32_t icmp_ping_timeout_ticks;
...
int icmp_ping_start(uint32_t target_ip, uint8_t count,
    ...
    if (!frequency ||
        frequency > ICMP_MAX_TICK_INTERVAL / timeout_seconds) {
        LOG_ERROR("NET", "Timer invalido para ping ICMP");
        return ERR_STATE;
    }
    ...
    icmp_status.last_error = OK;
    icmp_ping_timeout_ticks = frequency * timeout_seconds;
...
static int icmp_validate_active_configuration(void) {
    ...
    if (ipv4_get_status(&ipv4) != OK || !ipv4.configured ||
        ...
        icmp_status.last_error = ERR_STATE;
...
int icmp_maintain(void) {
    ...
    if ((uint32_t)(timer_get_ticks() - icmp_ping_sent_tick) >=
        icmp_ping_timeout_ticks) {
...
int icmp_validate_state(void) {
    ...
    if (!icmp_status.initialized) {
        if (icmp_status.state != ICMP_PING_IDLE ||
            icmp_pending_reply.active) {
            LOG_ERROR("NET", "ICMP ativo sem inicializacao");
            return ERR_STATE;
        }
        return OK;
    }
    if (ipv4_get_status(&ipv4) != OK || !ipv4.initialized ||
        icmp_status.state > ICMP_PING_FAILED ||
