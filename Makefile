...
HW6_QEMU_WORKERS ?= 4
HW6_QEMU_SEED ?= 2106
SHELL1_QEMU_WORKERS ?= 4
SHELL1_QEMU_SEED ?= 2201
SHELL2_QEMU_WORKERS ?= 4
SHELL2_QEMU_SEED ?= 2202
SHELL3_QEMU_WORKERS ?= 4
SHELL3_QEMU_SEED ?= 2203
SHELL4_QEMU_WORKERS ?= 4
SHELL4_QEMU_SEED ?= 2204
SHELL5_QEMU_WORKERS ?= 4
SHELL5_QEMU_SEED ?= 2205
SHELL6_QEMU_WORKERS ?= 4
SHELL6_QEMU_SEED ?= 2206
COVERAGE_BUILD_DIR ?= build-coverage
ASSEMBLY_RUN_ID ?= tst7-assembly-1
ASSEMBLY_TRACE_RUN_ID ?= tst7-assembly-trace-1
ASSEMBLY_BOOT_TRACE_RUN_ID ?= tst7-assembly-boot-5
ASSEMBLY_RECOVERY_TRACE_RUN_ID ?= tst7-recovery-1
...
DRIVER_LIFECYCLE_C = src/drivers/driver_lifecycle.c
DRIVER_LIFECYCLE_OBJ = $(BUILD_DIR)/driver_lifecycle.o
...
OBJS = $(ENTRY_OBJ) $(KERNEL_OBJ) $(PANIC_OBJ) $(LOG_OBJ) $(TEST_PROTOCOL_CORE_OBJ) $(TEST_PROTOCOL_OBJ) $(TEST_COVERAGE_OBJ) $(KERNEL_TESTS_OBJ) $(KERNEL_TESTS_PAGING_OBJ) $(KERNEL_TESTS_EXECUTION_OBJ) $(KERNEL_TESTS_STORAGE_OBJ) $(KERNEL_TESTS_NETWORK_OBJ) $(KERNEL_TESTS_PLATFORM_OBJ) $(KERNEL_TESTS_BLACKBOX_OBJ) $(KERNEL_TESTS_TST6_OBJ) $(KERNEL_TESTS_ASSEMBLY_OBJ) $(INPUT_OBJ) $(IRQ_DEFERRED_OBJ) $(WAIT_OBJ) $(WORKQUEUE_OBJ) $(SERVICE_SUPERVISOR_OBJ) $(RECOVERY_OBJ) $(CRYPTO_OBJ) $(CRYPTO_ED25519_OBJ) $(BEARSSL_COMPAT_OBJ) $(BEARSSL_OBJ) $(UPDATE_OBJ) $(UPDATE_SYSTEM_OBJ) $(UPDATE_SYSTEM_SLOTS_OBJ) $(UPDATE_REMOTE_SYSTEM_OBJ) $(UPDATE_REMOTE_OBJ) $(UPDATE_REMOTE_RELEASE_OBJ) $(UPDATE_REMOTE_GITHUB_OBJ) $(UPDATE_RUNTIME_OBJ) $(UPDATE_REMOTE_RUNTIME_OBJ) $(STRING_OBJ) $(APP_API_OBJ) $(SYSCALL_OBJ) $(SWITCH_OBJ) \
       ...
       $(MOUSE_OBJ) $(TIMER_OBJ) $(TSS_OBJ) $(ATA_OBJ) $(SPEAKER_OBJ) $(PCI_OBJ) $(UHCI_OBJ) $(EHCI_OBJ) $(USB_TRANSPORT_OBJ) $(USB_MSC_OBJ) $(USB_HID_OBJ) $(RTL8811CU_OBJ) $(E1000_OBJ) $(RTL8139_OBJ) $(AC97_OBJ) $(ACPI_OBJ) $(DRIVER_LIFECYCLE_OBJ) $(RNG_OBJ) \
...
$(KERNEL_OBJ): $(KERNEL_C) src/core/test_coverage.h src/include/apps/shell_job.h src/include/core/keyboard.h src/include/core/power.h src/include/core/input.h src/include/core/irq_deferred.h src/include/core/workqueue.h src/include/core/service_supervisor.h src/include/core/ethernet.h src/include/core/clock.h src/include/core/tls.h src/include/core/update_system.h src/include/core/update_system_slots.h src/include/core/update_remote_system.h src/include/core/test_protocol.h src/include/drivers/serial.h src/include/drivers/rtc.h src/drivers/driver_lifecycle_internal.h src/include/process/process.h src/include/process/thread.h src/include/memory/slab.h
	@if not exist $(BUILD_DIR) mkdir $(BUILD_DIR)
	$(GCC) $(CFLAGS) -c $< -o $@
...
$(DRIVER_LIFECYCLE_OBJ): $(DRIVER_LIFECYCLE_C) src/drivers/driver_lifecycle_internal.h src/include/core/errors.h src/include/types.h
	@if not exist $(BUILD_DIR) mkdir $(BUILD_DIR)
	$(GCC) $(CFLAGS) -c $< -o $@
...
$(VMA_OBJ): $(VMA_C) src/include/memory/vma.h src/include/process/process.h src/include/process/resource.h
	@if not exist $(BUILD_DIR) mkdir $(BUILD_DIR)
	$(GCC) $(CFLAGS) -c $< -o $@
...
$(STORAGE_OBJ): $(STORAGE_C) src/include/fs/storage.h src/include/fs/storage_internal.h src/include/fs/block_cache.h src/include/core/wait.h
	@if not exist $(BUILD_DIR) mkdir $(BUILD_DIR)
	$(GCC) $(CFLAGS) -c $< -o $@
...
$(SHELL_OBJ): $(SHELL_C) src/include/apps/shell_input.h src/include/apps/shell_dispatch.h src/include/apps/shell_command_utils.h src/include/apps/shell_job.h src/include/apps/shell_runtime.h src/include/core/keyboard.h src/include/core/string.h
	@if not exist $(BUILD_DIR) mkdir $(BUILD_DIR)
	$(GCC) $(CFLAGS) -c $< -o $@
...
$(SHELL_COMMANDS_STORAGE_OBJ): $(SHELL_COMMANDS_STORAGE_C) src/include/apps/shell.h src/include/apps/shell_dispatch.h src/include/apps/shell_command_utils.h src/include/apps/shell_job.h src/include/apps/shell_runtime.h src/include/fs/block_cache.h src/include/fs/storage_internal.h
	@if not exist $(BUILD_DIR) mkdir $(BUILD_DIR)
	$(GCC) $(CFLAGS) -c $< -o $@
...
$(SHELL_KMETRICS_OBJ): $(SHELL_KMETRICS_C) src/include/apps/shell_kmetrics.h src/include/apps/shell_runtime.h src/include/apps/shell_job.h src/include/core/ethernet.h src/include/core/errors.h src/include/core/input.h src/include/core/keyboard.h src/include/core/log.h src/include/core/memory.h src/include/core/network_manager.h src/include/core/recovery.h src/include/core/service_supervisor.h src/include/core/string.h src/include/core/timer.h src/include/core/update.h src/include/core/update_system_slots.h src/include/drivers/idt.h src/include/drivers/serial.h src/include/drivers/vesa.h src/include/fs/block.h src/include/fs/block_cache.h src/include/fs/permissions.h src/include/fs/vfs.h src/include/memory/paging.h src/include/process/credentials.h src/include/process/process.h src/include/process/resource.h
	@if not exist $(BUILD_DIR) mkdir $(BUILD_DIR)
	$(GCC) $(CFLAGS) -c $< -o $@
...
$(TASKMGR_OBJ): $(TASKMGR_C) src/include/apps/shell_introspection.h src/include/apps/taskmanager_test.h src/include/core/power.h src/include/fs/vfs.h src/include/process/process.h
	@if not exist $(BUILD_DIR) mkdir $(BUILD_DIR)
	$(GCC) $(CFLAGS) -c $< -o $@
...
$(APPSTORE_OBJ): $(APPSTORE_C) src/include/ui/appstore_test.h
	@if not exist $(BUILD_DIR) mkdir $(BUILD_DIR)
	$(GCC) $(CFLAGS) -c $< -o $@
...
test-qemu-parallel: $(OS_IMG) tools\qemu_parallel_runner.py tools\qemu_test_runner.py tests\catalog.json tests\unit\test_qemu_parallel_runner.py
	@if not exist "$(OS_IMG)" (echo Imagem ausente: $(OS_IMG) & exit /b 2)
...
test-hw2-qemu: $(OS_IMG) $(STORAGE_FIXTURES_STAMP) tools\qemu_parallel_runner.py tools\qemu_test_runner.py tests\catalog.json
	@if not exist "$(OS_IMG)" (echo Imagem ausente: $(OS_IMG) & exit /b 2)
	python tools\qemu_parallel_runner.py parallel --workers "$(HW2_QEMU_WORKERS)" --seed "$(HW2_QEMU_SEED)" --image "$(OS_IMG)" --catalog tests\catalog.json --results "$(BUILD_DIR)\test-results\hw2" --qemu $(QEMU) --cpu "$(QEMU_TEST_CPU)" --tag hw2

test-hw2: test-hw2-host test-hw2-qemu

test-hw3-qemu: $(OS_IMG) tools\qemu_parallel_runner.py tools\qemu_test_runner.py tests\catalog.json
	@if not exist "$(OS_IMG)" (echo Imagem ausente: $(OS_IMG) & exit /b 2)
	python tools\qemu_parallel_runner.py parallel --workers "$(HW3_QEMU_WORKERS)" --seed "$(HW3_QEMU_SEED)" --image "$(OS_IMG)" --catalog tests\catalog.json --results "$(BUILD_DIR)\test-results\hw3" --qemu $(QEMU) --cpu "$(QEMU_TEST_CPU)" --tag hw3

test-hw3: test-hw3-host test-hw3-qemu

test-hw4-qemu: $(OS_IMG) $(STORAGE_FIXTURES_STAMP) tools\qemu_parallel_runner.py tools\qemu_test_runner.py tests\catalog.json
	@if not exist "$(OS_IMG)" (echo Imagem ausente: $(OS_IMG) & exit /b 2)
	python tools\qemu_parallel_runner.py parallel --workers "$(HW4_QEMU_WORKERS)" --seed "$(HW4_QEMU_SEED)" --image "$(OS_IMG)" --catalog tests\catalog.json --results "$(BUILD_DIR)\test-results\hw4" --qemu $(QEMU) --cpu "$(QEMU_TEST_CPU)" --tag hw4 --storage-image "$(STORAGE_VALID_IMG)"

test-hw4: test-hw4-host test-hw4-qemu

test-hw5-qemu: $(OS_IMG) tools\qemu_parallel_runner.py tools\qemu_test_runner.py tests\catalog.json
	@if not exist "$(OS_IMG)" (echo Imagem ausente: $(OS_IMG) & exit /b 2)
	python tools\qemu_parallel_runner.py parallel --workers "$(HW5_QEMU_WORKERS)" --seed "$(HW5_QEMU_SEED)" --image "$(OS_IMG)" --catalog tests\catalog.json --results "$(BUILD_DIR)\test-results\hw5" --qemu $(QEMU) --cpu "$(QEMU_TEST_CPU)" --tag hw5

test-hw5: test-hw5-host test-hw5-qemu

test-hw6-qemu: $(OS_IMG) $(STORAGE_FIXTURES_STAMP) tools\qemu_parallel_runner.py tools\qemu_test_runner.py tests\catalog.json
	@if not exist "$(OS_IMG)" (echo Imagem ausente: $(OS_IMG) & exit /b 2)
	python tools\qemu_parallel_runner.py parallel --workers "$(HW6_QEMU_WORKERS)" --seed "$(HW6_QEMU_SEED)" --image "$(OS_IMG)" --catalog tests\catalog.json --results "$(BUILD_DIR)\test-results\hw6" --qemu $(QEMU) --cpu "$(QEMU_TEST_CPU)" --tag hw6

test-hw6: test-hw6-host test-hw6-qemu

test-shell1-qemu: $(OS_IMG) tools\qemu_parallel_runner.py tools\qemu_test_runner.py tests\catalog.json
	@if not exist "$(OS_IMG)" (echo Imagem ausente: $(OS_IMG) & exit /b 2)
	python tools\qemu_parallel_runner.py parallel --workers "$(SHELL1_QEMU_WORKERS)" --seed "$(SHELL1_QEMU_SEED)" --image "$(OS_IMG)" --catalog tests\catalog.json --results "$(BUILD_DIR)\test-results\shell1" --qemu $(QEMU) --cpu "$(QEMU_TEST_CPU)" --tag shell1

test-shell1: test-shell1-host test-shell1-qemu

test-shell2-qemu: $(OS_IMG) tools\qemu_parallel_runner.py tools\qemu_test_runner.py tests\catalog.json
	@if not exist "$(OS_IMG)" (echo Imagem ausente: $(OS_IMG) & exit /b 2)
	python tools\qemu_parallel_runner.py parallel --workers "$(SHELL2_QEMU_WORKERS)" --seed "$(SHELL2_QEMU_SEED)" --image "$(OS_IMG)" --catalog tests\catalog.json --results "$(BUILD_DIR)\test-results\shell2" --qemu $(QEMU) --cpu "$(QEMU_TEST_CPU)" --tag shell2

test-shell2: test-shell2-host test-shell2-qemu

test-shell3-qemu: $(OS_IMG) storage-fixtures tools\qemu_parallel_runner.py tools\qemu_test_runner.py tests\catalog.json
	@if not exist "$(OS_IMG)" (echo Imagem ausente: $(OS_IMG) & exit /b 2)
	python tools\qemu_parallel_runner.py parallel --workers "$(SHELL3_QEMU_WORKERS)" --seed "$(SHELL3_QEMU_SEED)" --image "$(OS_IMG)" --catalog tests\catalog.json --results "$(BUILD_DIR)\test-results\shell3" --qemu $(QEMU) --cpu "$(QEMU_TEST_CPU)" --tag shell3
...
test-shell5-qemu: $(OS_IMG) tools\qemu_parallel_runner.py tools\qemu_test_runner.py tests\catalog.json
	@if not exist "$(OS_IMG)" (echo Imagem ausente: $(OS_IMG) & exit /b 2)
	python tools\qemu_parallel_runner.py parallel --workers "$(SHELL5_QEMU_WORKERS)" --seed "$(SHELL5_QEMU_SEED)" --image "$(OS_IMG)" --catalog tests\catalog.json --results "$(BUILD_DIR)\test-results\shell5" --qemu $(QEMU) --cpu "$(QEMU_TEST_CPU)" --tag shell5

test-shell5: test-shell5-host test-shell5-qemu

test-shell6-qemu: $(OS_IMG) tools\qemu_parallel_runner.py tools\qemu_test_runner.py tests\catalog.json
	@if not exist "$(OS_IMG)" (echo Imagem ausente: $(OS_IMG) & exit /b 2)
	python tools\qemu_parallel_runner.py parallel --workers "$(SHELL6_QEMU_WORKERS)" --seed "$(SHELL6_QEMU_SEED)" --image "$(OS_IMG)" --catalog tests\catalog.json --results "$(BUILD_DIR)\test-results\shell6" --qemu $(QEMU) --cpu "$(QEMU_TEST_CPU)" --tag shell6

test-shell6: test-shell6-host test-shell6-qemu
...
test-assembly-boot-trace-qemu: coverage-image tools\assembly_boot_trace.py tools\assembly_trace_collector.py tests\catalog.json
	python tools\assembly_boot_trace.py --build-dir "$(COVERAGE_BUILD_DIR)" --catalog tests\catalog.json --output-dir "$(COVERAGE_BUILD_DIR)\test-results\assembly-boot-trace\$(ASSEMBLY_BOOT_TRACE_RUN_ID)" --nasm $(NASM) --nm "$(NM)" --compiler "$(GCC)" --qemu $(QEMU) --timeout 5

test-assembly-recovery-trace-qemu: coverage-image $(SYSTEM_BOOT_RETURN_BIN) tools\assembly_recovery_trace.py tools\assembly_trace_collector.py tests\catalog.json
	python tools\assembly_recovery_trace.py --build-dir "$(COVERAGE_BUILD_DIR)" --catalog tests\catalog.json --output-dir "$(COVERAGE_BUILD_DIR)\test-results\assembly-recovery-trace\$(ASSEMBLY_RECOVERY_TRACE_RUN_ID)" --system-boot-return "$(SYSTEM_BOOT_RETURN_BIN)" --nasm $(NASM) --ld "$(LD)" --nm "$(NM)" --compiler "$(GCC)" --qemu $(QEMU) --timeout 30
...
test-tst7-continuous-host: tools\tst7_continuous_runner.py tools\qemu_parallel_runner.py tests\unit\test_tst7_continuous_runner.py tests\unit\test_qemu_parallel_runner.py
	python -m unittest tests.unit.test_tst7_continuous_runner tests.unit.test_qemu_parallel_runner
...
test-tst7-continuous-parallel: tools\tst7_continuous_runner.py tools\qemu_parallel_runner.py tools\qemu_test_runner.py
	python tools\tst7_continuous_runner.py start --mode soak-parallel --forever --interval 60 --cycle-timeout "$(TST7_FULL_TIMEOUT)" --workers "$(QEMU_PARALLEL_WORKERS)" --qemu $(QEMU) --image "$(OS_IMG)" --catalog tests\catalog.json
...
test-update-system-slots-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_update_system_slots_host.c tests\catalog.json src\core\update_system_slots.c src\core\update_system_slots_host.h src\core\string.c
	python tools\core_host_runner.py --case host:core:update-system-slots --cc "$(HOST_CC)"

test-update-remote-system-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_update_remote_system_host.c tests\catalog.json src\core\update_remote_system.c src\core\update_remote_system_host.h src\core\string.c
	python tools\core_host_runner.py --case host:core:update-remote-system --cc "$(HOST_CC)"

test-update-remote-github-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_update_remote_github_host.c tests\catalog.json src\core\update_remote_github.c src\core\update_remote_github_host.h src\core\string.c
	python tools\core_host_runner.py --case host:core:update-remote-github --cc "$(HOST_CC)"
...
test-app-loader-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_app_loader_host.c tests\catalog.json src\core\app_loader.c src\include\core\app_loader.h src\include\core\app_api.h src\include\core\app_files.h
	python tools/core_host_runner.py --case host:core:app-loader --cc "$(HOST_CC)"
...
test-editor-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_editor_host.c tests\catalog.json src\shell\editor.c src\include\apps\editor.h src\include\apps\editor_test.h src\include\core\errors.h src\include\core\keyboard.h src\include\core\log.h src\include\core\memory.h src\include\core\recovery.h src\include\core\string.h src\include\core\timer.h src\include\core\video.h src\include\fs\fs.h src\include\process\process.h src\include\ui\desktop.h src\include\ui\taskbar.h
	python tools/core_host_runner.py --case host:shell:editor --cc "$(HOST_CC)"

test-settings-icons-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_settings_host.c tests\catalog.json src\settings\settings.c src\settings\settings_test.h src\include\ui\settings.h src\include\ui\icons.h src\include\ui\gui.h src\include\ui\display.h src\include\core\video.h src\include\core\log.h src\include\core\update_remote_system.h src\include\core\update_system_slots.h src\include\ui\taskbar.h
	python tools/core_host_runner.py --case host:ui:settings-icons --cc "$(HOST_CC)"

test-desktop-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_desktop_host.c tests\catalog.json src\desktop\desktop.c src\include\ui\desktop.h src\include\ui\display.h src\include\ui\gui.h src\include\ui\icons.h src\include\ui\taskbar.h src\include\drivers\vesa.h src\include\drivers\mouse.h src\include\drivers\font.h src\include\core\timer.h src\include\core\video.h src\include\core\errors.h src\include\core\log.h
	python tools/core_host_runner.py --case host:ui:desktop --cc "$(HOST_CC)"
...
test-storage-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_storage_host.c tests\catalog.json src\fs\storage.c src\include\fs\storage.h src\include\fs\storage_internal.h
	python tools\core_host_runner.py --case host:storage:storage --cc "$(HOST_CC)"

test-storage-fat32-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_storage_fat32_host.c tests\catalog.json src\fs\storage.c src\include\fs\storage.h src\include\fs\storage_internal.h
	python tools\core_host_runner.py --case host:storage:storage-fat32 --cc "$(HOST_CC)"
...
test-driver-lifecycle-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_driver_lifecycle_host.c tests\catalog.json src\drivers\driver_lifecycle.c src\drivers\driver_lifecycle_internal.h src\include\core\errors.h src\include\types.h
	python tools\core_host_runner.py --case host:drivers:lifecycle --cc "$(HOST_CC)"

test-hw2-host: test-hw1-host test-driver-lifecycle-host tools\qemu_test_runner.py tools\qemu_parallel_runner.py tests\catalog.json tests\coverage\registry.json
	python -m unittest tests.unit.test_qemu_test_runner tests.unit.test_qemu_parallel_runner

test-hw3-host: test-input-host test-keyboard-host test-mouse-host test-usb-hid-host test-vesa-host test-video-host test-ac97-host test-speaker-host test-shell-input-host test-shell-hosted-host test-guitest-host test-desktop-host test-driver-lifecycle-host test-kernel-host tools\qemu_test_runner.py tools\qemu_parallel_runner.py tests\unit\test_hw3_matrix.py tests\catalog.json tests\coverage\registry.json config\hardware-profiles.json
	python -m unittest tests.unit.test_hw3_matrix tests.unit.test_qemu_test_runner tests.unit.test_qemu_parallel_runner

test-hw4-host: test-ata-host test-uhci-host test-ehci-host test-usb-transport-host test-usb-manager-host test-usb-hid-host test-usb-msc-host test-block-host test-fat12-host test-fat32-host test-fs-host test-storage-host test-storage-fat32-host test-vfs-host test-vfs-path-host test-shell-diagnostics-host test-driver-lifecycle-host tests\unit\test_hw4_matrix.py tools\qemu_test_runner.py tools\qemu_parallel_runner.py tests\catalog.json tests\coverage\registry.json
	python -m unittest tests.unit.test_hw4_matrix tests.unit.test_qemu_test_runner tests.unit.test_qemu_parallel_runner

test-hw5-host: test-e1000-host test-rtl8139-host test-acpi-host test-power-host test-rtc-status-host test-network-manager-host test-network-host test-route-host test-ipv4-host test-udp-host test-arp-host test-icmp-host test-dns-host test-dhcp-host test-ethernet-host test-tcp-host test-http-host test-net-socket-host test-socket-runtime-host test-shell-network-checks-host test-driver-lifecycle-host test-kernel-host tests\unit\test_hw5_matrix.py tools\qemu_test_runner.py tools\qemu_parallel_runner.py tests\catalog.json tests\coverage\registry.json
	python -m unittest tests.unit.test_hw5_matrix tests.unit.test_qemu_test_runner tests.unit.test_qemu_parallel_runner

test-hw6-host: test-shell-diagnostics-host test-shell-checks-host test-shell-introspection-host test-device-manager-host test-acpi-host test-power-host test-network-manager-host test-usb-manager-host test-driver-lifecycle-host test-kernel-host tests\unit\test_hw6_matrix.py tests\unit\test_kernel_tests_blackbox_host.c tools\qemu_test_runner.py tools\qemu_parallel_runner.py tests\catalog.json tests\coverage\registry.json
	python -m unittest tests.unit.test_hw6_matrix tests.unit.test_qemu_test_runner tests.unit.test_qemu_parallel_runner
	python tools\core_host_runner.py --case host:tst5:blackbox --cc "$(HOST_CC)"

test-shell1-host: test-shell-core-host test-shell-input-host test-shell-hosted-host test-shell-job-host test-shell-dispatch-host test-blackbox-host tools\qemu_test_runner.py tools\qemu_parallel_runner.py tests\unit\test_shell1_matrix.py tests\catalog.json tests\coverage\registry.json
	python -m unittest tests.unit.test_shell1_matrix tests.unit.test_qemu_test_runner tests.unit.test_qemu_parallel_runner
	$(MAKE_TOOL) test-shell-core-host test-shell-input-host test-shell-hosted-host test-shell-job-host test-shell-dispatch-host test-blackbox-host

test-shell2-host: test-shell-core-host test-shell-commands-core-host test-shell-diagnostics-host test-shell-checks-host test-shell-introspection-host test-shell-commands-vfs-host test-shell-input-host test-shell-hosted-host test-shell-job-host test-shell-dispatch-host test-service-supervisor-host test-blackbox-host tools\qemu_test_runner.py tools\qemu_parallel_runner.py tests\unit\test_shell2_matrix.py tests\unit\test_kernel_tests_blackbox_host.c tests\catalog.json tests\coverage\registry.json
	python -m unittest tests.unit.test_shell2_matrix tests.unit.test_qemu_test_runner tests.unit.test_qemu_parallel_runner
	$(MAKE_TOOL) test-shell-core-host test-shell-commands-core-host test-shell-diagnostics-host test-shell-checks-host test-shell-introspection-host test-shell-commands-vfs-host test-shell-input-host test-shell-hosted-host test-shell-job-host test-shell-dispatch-host test-service-supervisor-host test-blackbox-host

test-shell3-host: test-shell-pipeline-host test-shell-commands-vfs-host test-shell-commands-storage-host test-file-index-host test-vfs-host test-vfs-path-host test-permissions-host test-storage-host test-storage-fat32-host test-filemanager-host test-taskmanager-host test-settings-icons-host test-desktop-host test-wm-host test-taskbar-host test-shell-hosted-host test-shell-input-host test-shell-commands-apps-host test-blackbox-host tools\qemu_test_runner.py tools\qemu_parallel_runner.py tests\unit\test_shell3_matrix.py tests\unit\test_kernel_tests_blackbox_host.c tests\catalog.json tests\coverage\registry.json
...
test-shell5-host: test-shell-commands-packages-host test-shell-commands-apps-host test-shell-job-host test-shell-hosted-host test-settings-icons-host test-updater-host test-blackbox-host tools\qemu_test_runner.py tools\qemu_parallel_runner.py tests\unit\test_shell5_matrix.py tests\unit\test_kernel_tests_blackbox_host.c tests\catalog.json tests\coverage\registry.json
	python -m unittest tests.unit.test_shell5_matrix tests.unit.test_qemu_test_runner tests.unit.test_qemu_parallel_runner
	$(MAKE_TOOL) test-shell-commands-packages-host test-shell-commands-apps-host test-shell-job-host test-shell-hosted-host test-settings-icons-host test-updater-host test-blackbox-host

test-shell6-host: test-shell-core-host test-shell-input-host test-shell-hosted-host test-shell-job-host test-shell-dispatch-host test-shell-commands-apps-host test-filemanager-host test-taskmanager-host test-settings-icons-host test-desktop-host test-wm-host test-taskbar-host test-guitest-host test-blackbox-host tools\qemu_test_runner.py tools\qemu_parallel_runner.py tests\unit\test_shell6_matrix.py tests\unit\test_kernel_tests_blackbox_host.c tests\catalog.json tests\coverage\registry.json
	python -m unittest tests.unit.test_shell6_matrix tests.unit.test_qemu_test_runner tests.unit.test_qemu_parallel_runner
	$(MAKE_TOOL) test-shell-core-host test-shell-input-host test-shell-hosted-host test-shell-job-host test-shell-dispatch-host test-shell-commands-apps-host test-filemanager-host test-taskmanager-host test-settings-icons-host test-desktop-host test-wm-host test-taskbar-host test-guitest-host test-blackbox-host
...
test-tls-client-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_tls_client_host.c tests\unit\host_include\bearssl.h tests\catalog.json src\core\tls_client.c src\include\core\tls_client.h
	python tools\core_host_runner.py --case host:security:tls-client --cc "$(HOST_CC)"
...
test-socket-runtime-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_socket_host.c tests\catalog.json src\core\socket.c src\include\core\socket.h src\include\core\net_socket.h src\include\core\sk_buff.h src\include\core\wait.h src\include\fs\vfs.h src\include\fs\vfs_internal.h
	python tools\core_host_runner.py --case host:network:socket-runtime --cc "$(HOST_CC)"

test-vma-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_vma_host.c tests\catalog.json src\memory\vma.c src\process\resource.c src\include\memory\vma.h src\include\process\resource.h
...
test-sysfs-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_sysfs_host.c tests\catalog.json src\fs\sysfs.c src\include\fs\sysfs.h src\include\fs\procfs.h src\include\fs\vfs.h src\include\core\errors.h src\include\core\log.h src\include\core\memory.h src\include\core\poll.h src\include\core\power.h src\include\core\spinlock.h src\include\core\string.h src\include\drivers\pci.h src\include\core\network_manager.h src\include\fs\block.h src\core\string.c
	python tools\core_host_runner.py --case host:storage:sysfs --cc "$(HOST_CC)"
...
test-mediaplayer-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_mediaplayer_host.c tests\catalog.json src\shell\mediaplayer.c src\include\apps\mediaplayer.h src\include\fs\wav.h src\include\fs\bmp.h src\include\fs\fs.h src\include\drivers\ac97.h src\include\drivers\vesa.h src\include\core\recovery.h
	python tools\core_host_runner.py --case host:shell:mediaplayer --cc "$(HOST_CC)"

test-shell-job-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_shell_job_host.c tests\catalog.json src\shell\shell_job.c src\include\apps\shell_job.h src\include\apps\shell.h src\include\apps\shell_command_utils.h src\include\apps\shell_runtime.h src\include\core\errors.h src\include\core\keyboard.h src\include\core\log.h src\include\core\timer.h src\include\core\video.h src\include\core\wait.h src\include\process\process.h
	python tools\core_host_runner.py --case host:shell:job --cc "$(HOST_CC)"

test-shell-pipeline-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_shell_pipeline_host.c tests\catalog.json src\shell\shell_pipeline.c src\include\apps\shell_pipeline.h src\include\apps\shell.h src\include\apps\shell_dispatch.h src\include\core\app_api.h src\include\core\errors.h src\include\core\log.h src\include\core\memory.h src\include\core\string.h src\include\core\video.h src\include\fs\vfs.h src\include\process\thread.h
	python tools\core_host_runner.py --case host:shell:pipeline --cc "$(HOST_CC)"
...
test-rng-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_rng_host.c tests\catalog.json src\drivers\rng.c src\include\drivers\rng.h src\include\core\errors.h src\include\core\log.h src\core\string.c
	python tools\core_host_runner.py --case host:drivers:rng --cc "$(HOST_CC)"

test-serial-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_serial_host.c tests\catalog.json src\drivers\serial.c src\include\drivers\serial.h src\include\core\log.h
	python tools\core_host_runner.py --case host:drivers:serial --cc "$(HOST_CC)"

test-tss-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_tss_host.c tests\catalog.json src\drivers\tss.c src\include\drivers\tss.h src\include\core\log.h src\include\core\memory.h src\core\string.c
	python tools\core_host_runner.py --case host:drivers:tss --cc "$(HOST_CC)"

test-speaker-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_speaker_host.c tests\catalog.json src\drivers\speaker.c src\include\drivers\speaker.h src\include\core\log.h src\include\core\timer.h
	python tools\core_host_runner.py --case host:drivers:speaker --cc "$(HOST_CC)"

test-keyboard-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_keyboard_host.c tests\catalog.json src\drivers\keyboard.c src\include\core\keyboard.h src\include\core\input.h src\include\core\irq_deferred.h src\include\drivers\idt.h src\include\core\log.h src\include\core\errors.h src\include\process\process.h src\core\string.c
	python tools\core_host_runner.py --case host:drivers:keyboard --cc "$(HOST_CC)"
...
test-coverage-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_coverage_host.c tests\catalog.json src\core\test_coverage.c src\core\test_coverage.h src\include\drivers\serial.h
	python tools\core_host_runner.py --case host:core:test-coverage --cc "$(HOST_CC)"
...
test-shell-hosted-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_shell_hosted_host.c tests\catalog.json src\shell\shell_hosted.c src\include\apps\shell.h src\include\apps\shell_runtime.h src\include\core\errors.h src\include\core\log.h src\include\core\video.h src\include\drivers\mouse.h src\include\ui\desktop.h src\include\ui\wm.h
	python tools\core_host_runner.py --case host:shell:hosted --cc "$(HOST_CC)"
...
test-shell-core-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_shell_host.c tests\catalog.json src\shell\shell.c src\core\string.c src\include\apps\shell.h src\include\apps\shell_input.h src\include\apps\shell_job.h src\include\apps\shell_runtime.h src\include\core\app_loader.h src\include\core\errors.h src\include\core\log.h src\include\core\string.h src\include\core\video.h src\include\drivers\mouse.h src\include\apps\guitest.h src\include\ui\appstore.h src\include\ui\desktop.h src\include\ui\filemanager.h src\include\ui\settings.h src\include\ui\taskbar.h src\include\ui\updater.h src\include\ui\wm.h
...
test-shell-commands-storage-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_shell_commands_storage_host.c tests\catalog.json src\shell\shell_commands_storage.c src\core\string.c src\include\apps\shell_job.h src\include\fs\file_index.h src\include\fs\storage_internal.h
	python tools\core_host_runner.py --case host:shell:commands-storage --cc "$(HOST_CC)"

test-shell-network-checks-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_shell_network_checks_host.c tests\catalog.json src\shell\shell_commands_network.c src\shell\shell_command_utils.c src\core\string.c src\include\apps\shell_job.h src\include\apps\shell_runtime.h src\include\apps\shell_command_utils.h src\include\core\arp.h src\include\core\dhcp.h src\include\core\dns.h src\include\core\errors.h src\include\core\ethernet.h src\include\core\http.h src\include\core\icmp.h src\include\core\ipv4.h src\include\core\log.h src\include\core\net_buffer.h src\include\core\net_socket.h src\include\core\network_manager.h src\include\core\poll.h src\include\core\recovery.h src\include\core\route.h src\include\core\sk_buff.h src\include\core\socket.h src\include\core\string.h src\include\core\tcp.h src\include\core\timer.h src\include\core\udp.h src\include\core\video.h src\include\core\wait.h src\include\drivers\idt.h src\include\fs\vfs.h src\include\process\thread.h
...
test-shell-diagnostics-host: src\include\core\net_buffer.h src\include\core\net_socket.h src\include\core\route.h src\include\core\sk_buff.h src\include\core\socket.h src\include\core\dhcp.h src\include\core\dns.h src\include\core\http.h src\include\core\ipv4.h src\include\core\tcp.h src\include\memory\slab.h
...
test-perf3-host: test-process-host test-workqueue-host test-shell-diagnostics-host test-blackbox-host tools\perf3_scheduler_idle.py tools\perf1_metrics.py tests\unit\test_perf3_scheduler_idle.py tests\unit\test_qemu_test_runner.py
	python -m unittest tests.unit.test_perf3_scheduler_idle tests.unit.test_qemu_test_runner

perf3-scheduler-idle: $(OS_IMG) tools\perf3_scheduler_idle.py tools\perf1_metrics.py tools\qemu_test_runner.py tests\catalog.json tests\coverage\registry.json
	@if not exist "$(OS_IMG)" (echo Imagem ausente: $(OS_IMG) & exit /b 2)
...
test-perf6-host: test-thread-host test-workqueue-host test-service-supervisor-host test-process-host test-scheduling-host test-shell-diagnostics-host test-blackbox-host tools\perf6_kworker_release.py tools\perf1_metrics.py tools\perf3_scheduler_idle.py tools\qemu_test_runner.py tests\unit\test_perf6_kworker_release.py tests\unit\test_qemu_test_runner.py
	python -m unittest tests.unit.test_perf6_kworker_release tests.unit.test_qemu_test_runner

perf6-kworker-thread: $(OS_IMG) tools\perf6_kworker_release.py tools\perf1_metrics.py tools\perf3_scheduler_idle.py tools\qemu_test_runner.py tests\catalog.json tests\coverage\registry.json
	@if not exist "$(OS_IMG)" (echo Imagem ausente: $(OS_IMG) & exit /b 2)
	python tools\perf6_kworker_release.py --qemu $(QEMU) --cpu "$(QEMU_TEST_CPU)" --network none --workers "$(PERF6_QEMU_WORKERS)"

test-perf6-qemu: perf6-kworker-thread

test-net1-host: test-dhcp-host test-dns-host test-tcp-host test-http-host test-network-host test-network-manager-host test-ethernet-host test-net-socket-host test-socket-runtime-host test-shell-network-checks-host test-shell-diagnostics-host tools\net1_external_connectivity.py tests\unit\test_net1_external_connectivity.py tools\qemu_test_runner.py
	$(PYTHON) -m unittest tests.unit.test_net1_external_connectivity tests.unit.test_qemu_test_runner

net1-external-connectivity: $(OS_IMG) tools\net1_external_connectivity.py tools\perf1_metrics.py tools\qemu_test_runner.py tests\catalog.json tests\coverage\registry.json
	@if not exist "$(OS_IMG)" (echo Imagem ausente: $(OS_IMG) & exit /b 2)
	$(PYTHON) tools\net1_external_connectivity.py --qemu $(QEMU) --cpu "$(QEMU_TEST_CPU)" --workers "$(NET1_QEMU_WORKERS)" --external-host "$(NET1_EXTERNAL_HOST)" --external-url "$(NET1_EXTERNAL_URL)" --network "$(NET1_QEMU_NETWORK)"

test-net1-qemu: net1-external-connectivity

perf6-release: $(OS_IMG) tools\updater.py
	@if not exist "$(OS_IMG)" (echo Imagem ausente: $(OS_IMG) & exit /b 2)
	python tools\updater.py audit-image --image "$(OS_IMG)" --expect-version 0.1.0

test-rls1-host: tools\release_baseline.py tests\unit\test_release_baseline.py
	$(PYTHON) -m unittest tests.unit.test_release_baseline

rls1-baseline: $(OS_IMG) $(KERNEL_ELF) $(RECOVERY_LOADER_BIN) $(RECOVERY_LOADER_PADDED_BIN) tools\release_baseline.py
	@if not exist "$(BUILD_DIR)\test-results\rls1-baseline" mkdir "$(BUILD_DIR)\test-results\rls1-baseline"
	$(PYTHON) tools\release_baseline.py collect --repo-root . --output "$(BUILD_DIR)\test-results\rls1-baseline\rls1-baseline.json" --image "$(OS_IMG)" --boot "$(BOOT_BIN)" --stage2 "$(STAGE2_BIN)" --kernel "$(KERNEL_BIN)" --recovery-loader "$(RECOVERY_LOADER_BIN)" --recovery-loader-padded "$(RECOVERY_LOADER_PADDED_BIN)" --kernel-elf "$(KERNEL_ELF)" --nm "$(NM)" --objdump "$(OBJDUMP)" --nasm "$(NASM)" --gcc "$(GCC)" --ld "$(LD)" --make "$(MAKE_TOOL)" --python "$(PYTHON)" --build-version 0.1.0 --target-version 1.0.0

test-rls2-host: test-shell2-host test-shell-checks-host test-taskmanager-host test-filemanager-host test-settings-icons-host test-wm-host test-app-loader-host tools\rls2_shell_liveness.py tools\perf1_metrics.py tests\unit\test_rls2_shell_liveness.py tests\unit\test_qemu_test_runner.py
	$(PYTHON) -m unittest tests.unit.test_rls2_shell_liveness tests.unit.test_qemu_test_runner

rls2-shell-liveness: $(OS_IMG) tools\rls2_shell_liveness.py tools\perf1_metrics.py tools\qemu_test_runner.py tests\catalog.json tests\coverage\registry.json
	@if not exist "$(OS_IMG)" (echo Imagem ausente: $(OS_IMG) & exit /b 2)
	$(PYTHON) tools\rls2_shell_liveness.py --qemu $(QEMU) --cpu "$(QEMU_TEST_CPU)" --network none --workers "$(RLS2_QEMU_WORKERS)"

test-rls2-qemu: rls2-shell-liveness

test-rls3-host: test-memory-host test-paging-host test-vma-host test-slab-host test-process-host test-process-resource-host test-process-signal-host test-process-ipc-host test-thread-host test-scheduling-host test-permissions-host test-spinlock-host test-shell-job-host test-workqueue-host test-vfs-host test-block-host test-route-host test-net-socket-host test-socket-runtime-host test-network-host test-network-manager-host test-ethernet-host test-service-supervisor-host test-updater-host test-update-system-host test-update-system-slots-host test-recovery-runtime-host test-recovery-menu-host test-recovery-loader-host test-shell-diagnostics-host test-blackbox-host tools\rls3_invariants.py tools\perf1_metrics.py tools\qemu_test_runner.py tests\unit\test_rls3_invariants.py tests\unit\test_qemu_test_runner.py
	$(PYTHON) -m unittest tests.unit.test_rls3_invariants tests.unit.test_qemu_test_runner

rls3-invariants: $(OS_IMG) tools\rls3_invariants.py tools\perf1_metrics.py tools\qemu_test_runner.py tests\catalog.json tests\coverage\registry.json
	@if not exist "$(OS_IMG)" (echo Imagem ausente: $(OS_IMG) & exit /b 2)
	$(PYTHON) tools\rls3_invariants.py --qemu $(QEMU) --cpu "$(QEMU_TEST_CPU)" --network none --workers "$(RLS3_QEMU_WORKERS)"

test-rls3-qemu: rls3-invariants

test-rls4-host: test-rls2-host test-rls3-host test-hw1-host test-hw3-host test-hw4-host test-hw5-host test-hw6-host test-shell5-host test-shell6-host test-sto7-host tools\rls4_supported_matrix.py tools\perf1_metrics.py tools\qemu_test_runner.py tests\unit\test_rls4_supported_matrix.py tests\unit\test_qemu_test_runner.py
	$(PYTHON) -m unittest tests.unit.test_rls4_supported_matrix tests.unit.test_qemu_test_runner

rls4-supported-matrix: $(OS_IMG) $(STORAGE_FIXTURES_STAMP) tools\rls4_supported_matrix.py tools\perf1_metrics.py tools\qemu_test_runner.py tests\catalog.json tests\coverage\registry.json
	@if not exist "$(OS_IMG)" (echo Imagem ausente: $(OS_IMG) & exit /b 2)
	$(PYTHON) tools\rls4_supported_matrix.py --qemu $(QEMU) --cpu "$(QEMU_TEST_CPU)" --workers "$(RLS4_QEMU_WORKERS)" --storage-image "$(STORAGE_VALID_IMG)" --no-space-image "$(STORAGE_NO_SPACE_IMG)"
...
test-gui-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_gui_host.c tests\catalog.json src\gui\gui.c src\include\ui\gui.h src\include\ui\display.h src\include\drivers\vesa.h src\include\drivers\font.h src\include\core\errors.h src\include\core\log.h src\include\core\string.h src\core\string.c
	python tools/core_host_runner.py --case host:gui:widgets --cc "$(HOST_CC)"

test-shell-commands-vfs-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_shell_commands_vfs_host.c tests\catalog.json src\shell\shell_commands_vfs.c src\shell\shell_command_utils.c src\include\apps\shell_command_utils.h src\include\apps\shell_pipeline.h src\include\core\errors.h src\include\core\log.h src\include\core\string.h src\include\core\video.h src\core\string.c
	python tools/core_host_runner.py --case host:shell:commands-vfs --cc "$(HOST_CC)"
...
test-kernel-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_kernel_host.c tests\catalog.json src\kernel\kernel.c src\drivers\driver_lifecycle.c src\drivers\driver_lifecycle_internal.h src\include\drivers\usb_hid.h src\core\kernel_host_test.h
...
test-idt-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_idt_host.c tests\catalog.json src\drivers\idt.c src\include\drivers\idt.h src\include\drivers\tss.h src\include\process\process.h src\include\process\signal.h src\include\core\errors.h src\include\core\log.h src\include\core\panic.h src\include\core\test_protocol.h src\include\core\video.h
	python tools/core_host_runner.py --case host:drivers:idt --cc "$(HOST_CC)"
...
test-vesa-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_vesa_host.c tests\catalog.json src\drivers\vesa.c src\include\drivers\vesa.h src\include\drivers\font.h src\include\core\memory.h src\include\core\log.h src\include\core\timer.h src\include\core\string.h
	python tools/core_host_runner.py --case host:drivers:vesa --cc "$(HOST_CC)"

test-video-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_video_host.c tests\catalog.json src\drivers\video.c src\include\core\video.h src\core\video_test.h src\include\drivers\vesa.h src\include\drivers\font.h src\include\drivers\mouse.h src\include\core\spinlock.h src\include\core\errors.h src\include\core\log.h
	python tools/core_host_runner.py --case host:drivers:video --cc "$(HOST_CC)"

test-acpi-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_acpi_host.c tests\catalog.json src\drivers\acpi.c src\include\drivers\acpi.h src\include\core\memory.h src\include\core\string.h src\include\core\errors.h src\include\core\log.h src\include\core\timer.h
	python tools/core_host_runner.py --case host:drivers:acpi --cc "$(HOST_CC)"
...
test-rtl8139-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_rtl8139_host.c tests\catalog.json src\drivers\rtl8139.c src\include\drivers\rtl8139.h src\include\core\ethernet.h src\include\core\irq_deferred.h src\include\core\memory.h src\include\core\string.h src\include\core\errors.h src\include\core\log.h src\include\core\timer.h src\include\drivers\idt.h src\include\drivers\pci.h
	python tools/core_host_runner.py --case host:drivers:rtl8139 --cc "$(HOST_CC)"

test-mouse-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_mouse_host.c tests\catalog.json src\drivers\mouse.c src\include\drivers\mouse.h src\include\core\input.h src\include\core\irq_deferred.h src\include\core\log.h src\include\core\errors.h src\include\drivers\idt.h src\include\drivers\vesa.h
	python tools/core_host_runner.py --case host:drivers:mouse --cc "$(HOST_CC)"

test-e1000-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_e1000_host.c tests\catalog.json src\drivers\e1000.c src\include\drivers\e1000.h src\include\core\ethernet.h src\include\core\errors.h src\include\core\irq_deferred.h src\include\core\memory.h src\include\core\string.h src\include\core\timer.h src\include\drivers\idt.h src\include\drivers\pci.h src\include\memory\paging.h
	python tools/core_host_runner.py --case host:drivers:e1000 --cc "$(HOST_CC)"
...
.PHONY: test-tst7-continuous-parallel
...
test-guitest-host: tools\core_host_runner.py tools\coverage_collector.py tests\unit\test_guitest_host.c tests\catalog.json src\shell\guitest_app.c src\include\apps\guitest.h src\include\apps\shell.h src\include\core\errors.h src\include\core\log.h src\include\core\recovery.h src\include\core\video.h src\include\drivers\mouse.h src\include\drivers\speaker.h src\include\drivers\vesa.h src\include\ui\desktop.h src\include\ui\display.h src\include\ui\gui.h src\include\ui\taskbar.h
	python tools\core_host_runner.py --case host:ui:guitest --cc "$(HOST_CC)"
