...
$(ETHERNET_OBJ): $(ETHERNET_C) src/include/core/ethernet.h src/include/core/net_buffer.h src/include/memory/slab.h
	@if not exist build mkdir build
	$(GCC) $(CFLAGS) -c $< -o $@
...
$(VFS_PATH_OBJ): $(VFS_PATH_C) src/include/fs/vfs.h src/include/fs/vfs_internal.h src/include/fs/devfs.h src/include/fs/storage.h src/include/process/process.h
	@if not exist build mkdir build
	$(GCC) $(CFLAGS) -c $< -o $@
