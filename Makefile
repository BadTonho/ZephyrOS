...
$(VFS_OBJ): $(VFS_C) src/include/fs/vfs.h src/include/process/process.h
	@if not exist build mkdir build
	$(GCC) $(CFLAGS) -c $< -o $@

$(VFS_PATH_OBJ): $(VFS_PATH_C) src/include/fs/vfs.h src/include/fs/vfs_internal.h src/include/fs/storage.h src/include/process/process.h
	@if not exist build mkdir build
	$(GCC) $(CFLAGS) -c $< -o $@
