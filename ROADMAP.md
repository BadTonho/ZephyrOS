...
VFS2 esta concluida e validada com App API 0.6, `chdir/getcwd`, namespace
universal, quatro montagens sincronizadas com Storage e `cwd` por processo.
...
VFS3 esta concluida e validada com App API 0.7, `devfs` fixo em `/dev`,
dispositivos de caractere e bloco, listagem universal e syscall 17 para
`ioctl`. As quatro vagas do Storage permanecem independentes da quinta
montagem virtual. VFS4 esta implementada com App API 0.8, syscall 18, pipes
...
**Documentação de atualização sincronizada em:** 2026-08-29 10:02 (America/Sao_Paulo).
...
| `wifi status|scan|connect <ssid>` | Diagnostica candidatos PCI e USB RTL8811CU sem inicializar radio Wi-Fi |
...
- [x] Matriz funcional no QEMU e encerramento formal da SYNC4; `R5` permanece
  pendente no Roadmap 09.
...
- [x] EP6.2 concluida em 2026-08-22 15:23 (America/Sao_Paulo): canal GitHub
  configuravel com BearSSL/HTTPS, descoberta por tag exata, fingerprint de
...
- [x] EP9.0A implementada e validada: pacote `ZSYS` separado do runtime,
  preflight somente leitura, Release combinada v2 e nenhum uso de slots ou
  reboot.
...
- [x] EP9.1 implementada e validada: slots A/B, estado/journal redundantes,
  staging local em chunks, marcador pendente, fixtures FAT32 e matriz QEMU
...
| Pós-1.0.0 | [`docs/roadmaps/17-migracao-gradual-rust.md`](docs/roadmaps/17-migracao-gradual-rust.md) | Migração gradual para um kernel híbrido C/Rust/Assembly após a versão 1.0.0. |
