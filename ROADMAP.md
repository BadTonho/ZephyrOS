...
### Fase 6D — contrato de console e ciclo de vida (em validação)

- [ ] `console_write` em blocos síncronos de até 1024 bytes, sem fila ou quota total.
- [ ] Códigos de saída: `0` para sucesso, não-zero como erro do app e `0xF120` reservado ao runtime.
- [ ] `app outputtest [fail]` valida nove blocos de 128 bytes e término normal com sucesso ou erro.
- [ ] Validar no QEMU saída, prompt único, `F12`, falha isolada e ausência de zumbis.
